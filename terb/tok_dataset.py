...
# 本文件为统计协方差与上下文生成提供数据预处理工具，涵盖token化、动态padding及mask展平等操作。
# TokenizedDataset将HF datasets条目转换为张量字典，便于直接喂入transformers模型。
class TokenizedDataset(Dataset):
    ...
    def __init__(self, text_dataset, tokenizer=None, maxlen=None, field="text"):
        # text_dataset: HF Dataset切片；tokenizer: transformers分词器；maxlen: 最大截断长度；field: 文本字段名
    ...
    def __getitem__(self, i):
        ...
        # position_ids按token顺序从0递增，attention_mask=1标记实际token，形状均为[len_sequence]
...
def dict_to_(data, device):
    ...
    """
    # 常用于将批量数据迁移到CUDA，保持与模型参数一致的device
    for k in data:
...
def make_padded_batch(items):
    ...
    """
    # pad_sequence按最长序列补齐，其输出张量形状为[len_batch, max_len]
...
def flatten_masked_batch(data, mask):
    ...
    """
    # data形状[batch, seq, hidden]，mask形状[batch, seq]；该函数拉平成[total_tokens, hidden]并仅保留mask为1的行
