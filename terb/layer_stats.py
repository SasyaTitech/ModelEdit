...
# STAT_TYPES用于根据配置字符串选择对应的统计量类：二阶矩、均值、范数均值等
STAT_TYPES = {
    "mom2": SecondMoment,
    "mean": Mean,
    "norm_mean": NormMean,
...
def main():
    ...
    # 通过命令行参数控制待采样模型、数据集、层号、统计类型及精度，默认配置与论文附录一致
...
def layer_stats(
    ...
    def get_ds():
        ...
        # 根据不同模型的config确定允许的最大序列长度，保证统计张量维度一致
        ...
        if batch_tokens is not None and batch_tokens < maxlen:
            maxlen = batch_tokens
        # TokenizedDataset会将文本转为(input_ids, position_ids, attention_mask)字典
    ...
    # CombinedStat会根据to_collect聚合多种统计量，内部维护各项RunningStat
    stat = CombinedStat(**{k: STAT_TYPES[k]() for k in to_collect})
    # tally会遍历数据集并即时更新stat，返回的生成器会根据batch_size产出若干批次
    loader = tally(
        stat,
        ds,
    ...
    with torch.no_grad():
        for batch_group in progress(loader, total=batch_count):
            for batch in batch_group:
                ...
                # Trace上下文管理器会记录指定层的输入/输出，本处只关心输入(即第二层MLP的输入表示)
                with Trace(
                    model, layer_name, retain_input=True, retain_output=False, stop=True
                ) as tr:
                    model(**batch)
                # flatten_masked_batch会根据attention_mask过滤padding token，只保留有效token表示
